...
```{r marital_status, echo=FALSE, fig.cap='Marital Status and Income'}
age %>%
...
```{r decision_on_giving, echo=FALSE}
dg <- gss %>% 
...
```{r reasons_for_giving, echo=FALSE}
...
for (r in colnames(reasons_for_giving)){
  counted<-
    reasons_for_giving %>%
    count(.data[[r]])
  rg<-rbind(rg, c(rg_lst[i], counted$n))
  i<-i+1
}
rg<-rg[-c(1),]
...
```{r reasons_for_not_giving, echo=FALSE}
...
for (r in colnames(reasons_for_not_giving)){
  counted<-
    reasons_for_not_giving %>%
    count(.data[[r]])
  ng<-rbind(ng, c(ng_lst[i], counted$n))
  i<-i+1
}
ng<-ng[-c(1),]
ng %>% 
